try {
  ...
  console.error([
    ...
    "and `underscore.string`. they aren't in the package.json as a dependency ",
    "because nobody really cares about markdown in the terminal apparently. ",
    "Install them with",
    "`npm install colors wordwrap underscore.string`."
  ].join('\n'));
  process.exit(1);
}
...
exports.setOptions = function (options) {
  if (options.cols) {
    width = options.cols;
    wrap = wordwrap(3, width);
    indent = wordwrap(9, width);
    paragraph = wordwrap(6, width);
  }
};
